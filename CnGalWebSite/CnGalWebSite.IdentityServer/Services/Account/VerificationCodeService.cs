...
namespace CnGalWebSite.IdentityServer.Services.Account
{
    public class VerificationCodeService:IVerificationCodeService
    {
        ...
        public async Task<int> GetCodeAsync(string token, string userName,VerificationCodeType type)
        {
            var now = DateTime.UtcNow;


            //查找是否存在令牌
            var code = await _verificationCodeRepository.GetAll().OrderByDescending(s => s.Time).FirstOrDefaultAsync(s => s.UserName == userName && s.Type == type && (token == null || (token == s.Token && s.Time.AddMinutes(_maxTimeSpan) > now)));
            if (code != null)
            {
                return code.Code;
            }
        ...
        public async Task<string> GetTokenAsync(string code, string userName, VerificationCodeType type)
        {
            var now = DateTime.UtcNow;
            var token = await _verificationCodeRepository.FirstOrDefaultAsync(s => s.UserName == userName && s.Type == type && s.Code.ToString() == code && s.Time.AddMinutes(_maxTimeSpan) > now);
            if (token == null)
            {
                return null;
            }
