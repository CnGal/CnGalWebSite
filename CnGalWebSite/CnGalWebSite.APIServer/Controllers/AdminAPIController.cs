...
namespace CnGalWebSite.APIServer.Controllers
{
    ...
    public class AdminAPIController : ControllerBase
    {
        ...
        public async Task<ActionResult<Result>> TempFunction()
        {
            try
            {
                var steamIds = new int[] { 924540, 1082870, 820770, 962940, 1020060, 1082880, 943100, 895920, 1036690, 1117650, 1058040, 1007450 , 700470 };
                ...
                _ = await _steamInforRepository.GetRangeUpdateTable().Where(s => steamIds.Contains(s.SteamId)).Set(s => s.OriginalPrice, b => -3).ExecuteAsync();
                return new Result { Successful = true };
            }
            catch (Exception ex)
            {
